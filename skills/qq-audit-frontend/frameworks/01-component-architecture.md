...
|-----------|----------------------------------------|
| **State Management** | Where state lives determines component boundaries. A component that owns state it should not own will resist decomposition. Audit state placement alongside component structure. |
| **Render Performance** | God components re-render everything when any piece of their state changes. Poor composition directly causes poor render performance. |
| **CSS Architecture** | Component-scoped styles only work when component boundaries are clean. A god component with 300 lines of CSS is a god stylesheet too. |
| **TypeScript Strictness** | Typed prop interfaces make component contracts explicit and auditable. Without types, you cannot evaluate prop surface area — you can only guess. |
| **Code Organization** | File structure should reflect component hierarchy, not contradict it. If atoms live in random directories, the architecture exists only in theory. |
| **Error Boundaries** | Error boundaries are components. They need to be placed at meaningful composition boundaries — which only exist if the architecture has boundaries to begin with. |
| **Technical Debt** | Every god component, every copy-paste twin, every boolean hydra is technical debt. Component architecture quality is a leading indicator of overall codebase health. |
