...
|-----------|------------------------------|
| **Render Performance** | Bundle size affects time-to-interactive. A large bundle delays the first render and the first interaction. But after initial load, render performance is independent of bundle size. |
| **Component Architecture** | Well-structured components with clear boundaries are easier to code-split. A god component that imports everything cannot be split. |
| **Dependency Health** | Every dependency is a bundle size cost. Dependency health (maintained, minimal, justified) and bundle size are two views of the same problem. |
| **Build Pipeline** | The build pipeline is where tree shaking, minification, compression, and code splitting happen. Bundle size is a build pipeline output metric. |
| **SSR/Hydration** | SSR can mask bundle size problems for initial load. But hydration requires the full JS bundle to execute, so the cost is deferred, not eliminated. |
| **CSS Architecture** | CSS-in-JS adds JS bundle size. Utility-first CSS (Tailwind) adds CSS bundle size. The choice of CSS approach directly impacts the JS performance budget. |
