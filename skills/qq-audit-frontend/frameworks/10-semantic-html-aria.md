...
|-----------|-------------------------------|
| **Component Architecture** | Components that use semantic HTML internally are accessible by default. A `<Button>` component that renders `<button>` inherits native accessibility. A `<Button>` that renders `<div>` must manually implement everything. |
| **Form Handling** | Form accessibility requires labels, error messages, required indicators, and submission feedback. Every form handling pattern should include its accessibility dimension. |
| **CSS Architecture** | Focus indicators, contrast ratios, and visual-only information are CSS concerns with accessibility implications. CSS architecture should include accessibility as a design constraint. |
| **Routing Architecture** | Route transitions need focus management. After navigation, focus should move to the new content. Without this, keyboard and screen reader users are "lost" after every page change. |
| **Error Boundaries** | Error fallback UI must be accessible. A fallback that appears on screen but is not announced to screen readers does not help assistive technology users. |
| **i18n Readiness** | Language attributes (`lang` on elements), RTL text direction, and locale-specific screen reader behavior intersect with accessibility. |
