...
|-----------|-----------------------------------|
| **Test Pyramid** | Contract tests fill the "hollow middle" — they're faster than e2e and more realistic than unit tests for inter-service interactions. |
| **Error Scenario Simulation** | Contracts should define error responses, not just success responses. A contract for "what does 401 look like?" is as important as "what does 200 look like?" |
| **Happy/Sad Path** | Contracts tend to cover happy paths only. Add contracts for error responses: what does the provider return when the resource doesn't exist? When auth fails? |
| **Test Data Management** | Provider state setup for contract verification needs realistic data. Test data factories drive contract verification fixtures. |
| **Feature Flag Testing** | If a feature flag changes API behavior, the contract should specify behavior for both flag states. Otherwise, the flag change breaks consumers silently. |
| **Data Migration Testing** | Schema migrations can break contracts. Run contract verification after every migration to catch incompatibilities before deployment. |
