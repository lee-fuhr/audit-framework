...
|-----------|-------------------------------|
| **Test Pyramid** | Coverage should be tracked per pyramid layer. Unit coverage should be highest; e2e coverage contribution should be minimal. If removing e2e tests craters coverage, the base is hollow. |
| **Mutation Testing** | Mutation testing is the antidote to meaningless coverage. If coverage is high but mutation kill rate is low, the tests execute code without detecting changes. Use together. |
| **Boundary Value Analysis** | Boundary conditions are the highest-value coverage targets. If branch coverage is high but boundary values aren't tested, the coverage is superficially broad but substantively shallow. |
| **Test Readability** | Readable tests with clear assertions naturally produce meaningful coverage. Unreadable tests tend toward shallow "call and forget" patterns that inflate numbers. |
| **Happy/Sad Path** | Coverage distribution across happy and sad paths reveals test intent. If 90% of coverage comes from happy paths, the sad path code is the real uncovered area. |
| **Regression Effectiveness** | Escaped defects should be cross-referenced with coverage data. If the escaped bug was in "covered" code, the coverage was meaningless for that path. |
