...
|-----------|---------------------------------------------|
| **Happy/Sad Path** | Error simulation IS the mechanism for testing sad paths at the dependency layer. Without simulation, dependency sad paths can't be tested in isolation. |
| **Chaos/Resilience Testing** | Error simulation is the unit-test version of chaos engineering. Chaos testing is error simulation at infrastructure scale. |
| **Contract Testing** | Error contracts (what does the dependency return on failure) should be part of the contract. Contract tests should include error response contracts. |
| **Boundary Value Analysis** | Timeout boundaries, rate limit boundaries, and payload size boundaries are all boundary value tests applied to dependency interactions. |
| **Test Isolation** | Error simulation requires mocking, which gives dependency isolation. The same mock infrastructure serves isolation and fault injection. |
| **Smoke/Sanity Suite** | Post-deployment smoke tests should verify that error handling works in the deployed environment: can the system handle a dependency being slow? |
