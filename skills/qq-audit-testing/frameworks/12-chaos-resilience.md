...
|-----------|-------------------------------------------|
| **Load/Stress Testing** | Load testing measures capacity under ideal conditions. Chaos testing measures capacity under degraded conditions. The combination (load + failure injection) is the most realistic test. |
| **Error Scenario Simulation** | Error simulation at the application level (mock returning errors) is the unit-test version of chaos testing. Chaos testing is error simulation at the infrastructure level. |
| **Happy/Sad Path** | Chaos testing is sad path testing at system scale. Every chaos experiment tests a failure mode that functional sad path tests can't reach. |
| **Smoke/Sanity Suite** | A post-deployment smoke test should verify that resilience mechanisms (circuit breakers, fallbacks) are correctly configured in the new deployment. |
| **Contract Testing** | Dependency failures that chaos testing reveals often stem from contract violations under stress — the dependency returns a different error format than expected. |
| **CI Pipeline Speed** | Chaos experiments are too slow and unpredictable for per-PR CI. Run them as scheduled exercises (weekly game days, monthly chaos experiments). |
