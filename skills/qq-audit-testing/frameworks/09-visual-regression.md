...
|-----------|---------------------------------------------|
| **Cross-Browser/Device Testing** | Visual regression testing IS cross-browser testing for appearance. Functional cross-browser testing verifies behavior; visual regression testing verifies rendering. |
| **Accessibility Testing** | Some visual regressions are accessibility violations (insufficient contrast after a color change, text that overflows and becomes unreadable). Visual testing catches the visual symptom; accessibility testing diagnoses the compliance impact. |
| **CI Pipeline Speed** | Visual tests are slow (render + screenshot + compare). They must be parallelized and potentially run as a separate CI stage. If they block the main pipeline for 15 minutes, developers will disable them. |
| **Test Data Management** | Visual test consistency depends on data consistency. If test data changes between runs, visual diffs reflect data changes, not UI changes. Use fixed test data for visual testing. |
| **Feature Flag Testing** | Feature flags that change UI appearance need visual tests for BOTH flag states. A flag that shows a new banner should have baselines for banner-on and banner-off. |
| **Smoke/Sanity Suite** | A post-deployment visual smoke test (screenshot the 5 most critical pages and compare) catches the most visible regressions in under 30 seconds. |
