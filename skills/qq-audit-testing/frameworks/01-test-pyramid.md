...
|-----------|-------------------------------|
| **Code Coverage** | Coverage metrics should be tracked per layer. 95% unit coverage with 20% integration coverage exposes the hollow middle. |
| **Test Isolation** | Isolation is easiest at the base and hardest at the top. Pyramid shape directly determines how much isolation infrastructure you need. |
| **CI Pipeline Speed** | The pyramid is the primary lever for CI speed. An inverted pyramid makes fast CI physically impossible. |
| **Flaky Test Detection** | Flakiness concentrates at the top. A healthy pyramid shape naturally reduces flaky test volume. |
| **Contract Testing** | Fills the hollow middle in distributed architectures. Should be treated as a dedicated layer between integration and e2e. |
| **Mutation Testing** | Most effective at the unit layer. If mutation testing shows low kill rates, the base of the pyramid is hollow — tests exist but don't detect bugs. |
| **Smoke/Sanity Suite** | A well-designed smoke suite is the curated tip of the pyramid — the minimal e2e set that must pass before any deployment. |
