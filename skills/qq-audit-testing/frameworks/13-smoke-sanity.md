...
|-----------|----------------------------------------|
| **CI Pipeline Speed** | Smoke tests are the speed-critical path of the deployment pipeline. They must be the fastest tests in the system. |
| **Load/Stress Testing** | A lightweight load test (10 concurrent users, 30 seconds) can double as a smoke test for basic performance validation. |
| **Visual Regression** | A visual smoke test (screenshot 3 critical pages and compare) catches the most visible CSS regressions in under 30 seconds. |
| **Test Isolation** | Smoke tests must be isolated from each other AND from prior runs. Shared state between smoke test runs causes false positives. |
| **Error Scenario Simulation** | Smoke tests should verify the happy path. Error scenario tests verify the sad paths. Both are needed post-deployment but smoke tests come first. |
| **Cross-Browser/Device** | Smoke tests typically run in one browser. If a deployment breaks a specific browser, smoke tests won't catch it. Consider browser-specific smoke tests for critical paths. |
