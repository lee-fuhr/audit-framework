...
|-----------|-----------------------------------|
| **Test Pyramid** | Unit tests should be the most readable (simplest setup, clearest assertions). E2e tests are inherently harder to make readable due to their complexity. |
| **Code Coverage** | Readable tests with clear assertions naturally produce meaningful coverage. Unreadable tests tend toward shallow assertions that inflate coverage without catching bugs. |
| **Mutation Testing** | Readable tests with specific assertions produce high mutation kill rates. Tests with vague assertions (`toBeTruthy`) let mutants survive. |
| **Boundary Value Analysis** | Boundary tests should clearly state the boundary in the name: `rejects_quantity_zero`, `accepts_maximum_length_255`. BVA readability comes from the name. |
| **Test Data Management** | Factory-based data creation improves readability by hiding incidental details. The test shows only the data that matters for this specific scenario. |
| **Regression Effectiveness** | Regression tests should be the most readable tests in the suite because they document bugs. A regression test that future developers can't understand is a regression test that will be accidentally deleted. |
