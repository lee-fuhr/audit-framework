...
|-----------|------------------------------------------------|
| **Visual Regression Testing** | Visual regression tests should run per browser. A CSS regression that only appears in Safari won't be caught by Chrome-only visual tests. |
| **Accessibility Testing** | Accessibility behavior varies by browser and assistive technology combination. Screen reader + browser pairings (NVDA + Firefox, VoiceOver + Safari, JAWS + Chrome) each behave differently. |
| **Smoke/Sanity Suite** | A multi-browser smoke test after deployment catches the highest-impact cross-browser regressions: run the critical path in Chrome, Safari, and Firefox. |
| **CI Pipeline Speed** | Multi-browser testing multiplies CI time. Parallelize across browsers and consider running full matrix nightly while running primary browser per-PR. |
| **Load/Stress Testing** | Client-side performance varies by browser. JavaScript-heavy applications should be load-tested with real browser rendering (not just HTTP load). |
| **Feature Flag Testing** | Feature flags that change UI or CSS should be tested across browsers. A feature that works in Chrome but breaks Safari on flag-on is a cross-browser regression hidden behind a flag. |
