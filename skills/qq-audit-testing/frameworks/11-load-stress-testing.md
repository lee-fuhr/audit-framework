...
|-----------|---------------------------------------|
| **Chaos/Resilience Testing** | Load testing measures capacity under ideal conditions. Chaos testing measures capacity when things are broken. Combine them: load test WHILE injecting failures. |
| **Smoke/Sanity Suite** | A lightweight load test (10 concurrent users, 60 seconds) makes an excellent smoke test for performance regressions after deployment. |
| **Error Scenario Simulation** | Load testing should include error scenarios: what happens under load when the database is slow? When a dependency returns errors? |
| **CI Pipeline Speed** | Full load tests are too slow for per-PR CI. But a lightweight baseline test (30 seconds, low concurrency) can run per-PR to catch egregious regressions. |
| **Test Data Management** | Load test data volume must match production. A test database with 100 rows will have radically different query performance than production with 10M rows. |
| **Regression Effectiveness** | Performance regressions are regressions too. Track p95 response time across deployments like you track functional defect escape rate. |
